...
listtokens(FILE * f, tokenrec * l_buf)
{
	...
	while (l_buf != NULL)
	{
		...
		case tokvar:
			...
			PhreeqcPtr->output_temp_msg(PhreeqcPtr->sformatf("%s", l_buf->UU.vp->name));
			break;
		...
		case toknum:
			...
			output_temp_msg(PhreeqcPtr->sformatf("%s", string));
			break;

		case tokstr:
			output_temp_msg(PhreeqcPtr->sformatf("\"%s\"", l_buf->UU.sp));
			break;

		case toksnerr:
			output_temp_msg(PhreeqcPtr->sformatf("{%c}", l_buf->UU.snch));
			break;
		...
		case tokrem:
			output_temp_msg(PhreeqcPtr->sformatf("REM%s", l_buf->UU.sp));
			break;
	...
	do
	{
		...
		while (l != NULL && l->num <= n2)
		{
			if (l->num >= n1)
			{
				...
				/* putchar('\n'); */
				output_temp_msg(PhreeqcPtr->sformatf("%ld ", l->num));
...
/*      fputs(n.UU.sval, stdout); */
			output_temp_msg(PhreeqcPtr->sformatf("%s ", n.UU.sval));
			PHRQ_free(n.UU.sval);
		}
		else
/*      printf("%s ", numtostr(STR1, n.UU.val)); */
			output_temp_msg(PhreeqcPtr->sformatf("%s ", numtostr(STR1, n.UU.val)));
	}
	if (!semiflag)
/*    putchar('\n');*/
		output_temp_msg("\n");
