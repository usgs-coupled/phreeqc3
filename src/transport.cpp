...
LDBLE dV_dcell; // difference in Volt * F/RT among icell and jcell
int find_current;
 ...
 */
	...
	for (i = 1; i <= count_cells; i++)
	{
		...
		else
		{
			...
			//if (use.Get_solution_ptr()->Get_potV())
			//{
			//	cell_data[i].potV = use.Get_solution_ptr()->Get_potV();
			//	if (!dV_dcell)
			//		dV_dcell = 1;
			//}
		}
	...
	if (multi_Dflag)
	{
		...
		ct = (struct CT *) PHRQ_malloc((size_t)
			 (all_cells) *
			 sizeof(struct CT));
		if (ct == NULL)
			malloc_error();
 ...
 */
	for (n = 1; n <= stag_data->count_stag; n++)
	{
		for (i = 1; i <= count_cells; i++)
		{
			...
			// if (use.Get_solution_ptr()->Get_potV())
			// {
				// cell_data[k].potV = use.Get_solution_ptr()->Get_potV();
				// if (!dV_dcell)
					// dV_dcell = 1;
			// }
		}
...
/* ---------------------------------------------------------------------- */
{
	...
	//for(i = 0; i < count_cells + 1; i++)
	//{
	//	m.push_back(0); // this takes 5* more time than malloc & m[i] = 0
	//	m1.push_back(0);
	//}
	LDBLE *m, *m1;
	...
	{
		if (dV_dcell)
			dV_dcell = (cell_data[count_cells + 1].potV - cell_data[0].potV) / (count_cells + 1);
		for (i = 1; i < count_cells; i++)
		{
...
/* ---------------------------------------------------------------------- */
{
	 ...
	 * If an electrical field is applied (dV_dcell != 0), the currents j_i = current_cells[i].ele (C * s)
			...
			the relative cell resistance, R_i / sum_R, and the smallest j_x is the current_x (C * s),
			equal for all cells.
	 */
		...
		for (n = 0; n < (stagnant ? stag_data->count_stag : 1); n++)
		{
			...
			for (i = first_c; i <= last_c; i++)
			{
				...
				if (find_current)
				{
					...
					else
					{
						...
						for (i1 = 0; i1 <= count_cells; i1 ++)
						{
							...
							if (rate_sim_time > 20 * 3600) //debug
								i1 += 0;
				...
				if (i > 0 || stagnant || (i == 0 && dV_dcell))
				{
					...
					use.Get_solution_ptr()->Set_cb(use.Get_solution_ptr()->Get_cb() - ct[icell].J_ij_sum);
				...
				if (i < count_cells || stagnant || (i == count_cells && dV_dcell))
				{
					...
					use.Get_solution_ptr()->Set_cb(dummy + ct[icell].J_ij_sum);
					for (l = 0; l < count_m_s; l++)
					{
						temp = 0.0;
						length = (int) strlen(m_s[l].name);
...
/*	{
	...
	// LDBLE dl_s, A_ij, Dz2c, Dz2c_dl, visc1, visc2 and struct V_M made global in structure ct
	...
	if (dl_aq1 > 0)
		ct[icell].dl_s = dl_aq1 / (dl_aq1 + aq1);
	if (dl_aq2 > 0)
	{
	...
	{							/* regular column... */
		...
		else
		{
			...
			A1 = (aq1 + dl_aq1) / (cell_data[icell].length *
						0.5 * cell_data[icell].length);
			A2 = (aq2 + dl_aq2) / (cell_data[jcell].length *
						0.5 * cell_data[jcell].length);
			if (il_calcs)
			{
				dum = A1 * por_il12 /
	...
	while (i < i_max || j < j_max)
	{
						  ...
						  sol_D[jcell].spec[j].name) < 0))
		{
			...
			else
			{
				...
				ct[icell].v_m[k].Dzc = ct[icell].v_m[k].Dz * sol_D[icell].spec[i].c / 2;
				if (ct[icell].dl_s > 0)
				{
					s_charge_ptr = (dl_aq1 > 0) ? s_charge_ptr1 : s_charge_ptr2;
					LDBLE g = s_charge_ptr->Get_g_map()[ct[icell].v_m[k].z].Get_g();
					{
						{
							if (only_counter)
							{
								if ((s_charge_ptr->Get_la_psi() < 0 && ct[icell].v_m[k].z < 0)
								...
								{
									ct[icell].v_m[k].g_dl = 1.0;
									ct[icell].v_m[k].Dzc_dl =
										ct[icell].v_m[k].Dz * sol_D[icell].spec[i].c /
										2;
								}
							}
							else
							{
								if (dl_aq1 > 0)
								{
									...
									ct[icell].v_m[k].Dzc_dl =
										ct[icell].v_m[k].Dz * sol_D[icell].spec[i].c /
										2 * ct[icell].v_m[k].g_dl;
								}
								else
									ct[icell].v_m[k].Dzc_dl =
										ct[icell].v_m[k].Dz * sol_D[icell].spec[i].c / 2;
							}
							//break;
						}
					}
							   ...
							   sol_D[jcell].spec[j].name) > 0))
		{
			...
			else
			{
				...
				ct[icell].v_m[k].Dz = ct[icell].v_m[k].D * ct[icell].v_m[k].z;
				ct[icell].v_m[k].Dzc = ct[icell].v_m[k].Dz * sol_D[jcell].spec[j].c / 2;
				if (ct[icell].dl_s > 0)
				{
					...
					{
						{
							if (only_counter)
							{
								...
								else	/* assume for counter ions in the DDL the free pore space conc's... */
								{
									ct[icell].v_m[k].g_dl = 1.0;
									ct[icell].v_m[k].Dzc_dl =
										ct[icell].v_m[k].Dz * sol_D[jcell].spec[j].c /
										2;
								}
							}
							else
				 ...
				 == 0)
		{
			...
			else
			{
				...
				if (ct[icell].dl_s > 0)
				{
					...
					if (dl_aq1 > 0)
					{
						LDBLE g = s_charge_ptr1->Get_g_map()[ct[icell].v_m[k].z].Get_g();
						{
							{
								if (only_counter)
								{
					...
					if (dl_aq2 > 0)
					{
						LDBLE g = s_charge_ptr2->Get_g_map()[ct[icell].v_m[k].z].Get_g();
						{
							{
								if (only_counter)
								{
	...
	// dV_dcell, 2nd pass, current_x has been calculated, but needs V_M, dl_s, A_ij, Dz2c, Dz2c_dl, J_ij_count_spec, visc1, visc2...
dV_dcell2 :
	...
	{
		c += ct[icell].v_m[i].Dz * ct[icell].v_m[i].grad;
	...
	if (dV_dcell  && !find_current)
	{
		...
		for (i = 0; i < ct[icell].J_ij_count_spec; i++)
			current_cells[icell].ele -= ct[icell].v_m[i].Dzc * dV * F_Re3 / tk_x2 *
			(1 - ct[icell].dl_s) * ct[icell].v_m[i].z;
		dV *= current_x / (current_cells[icell].ele * ct[icell].A_ij * DDt);
	}
	for (i = 0; i < ct[icell].J_ij_count_spec; i++)
	{
		ct[icell].J_ij[i].tot1 = -ct[icell].v_m[i].D * ct[icell].v_m[i].grad + 
			c * ct[icell].v_m[i].Dzc / ct[icell].Dz2c;
		...
		if (ct[icell].Dz2c_dl > 0)
		{
			...
			if ((ct[icell].J_ij[i].tot1 <= 0 && dum <= 0)
				|| (ct[icell].J_ij[i].tot1 > 0 && dum > 0))
			{
		...
		if (find_current)
		{
			//current_cells[icell].dif -= ct[icell].v_m[i].D * ct[icell].v_m[i].grad * 
			//	(1 - ct[icell].dl_s) * ct[icell].v_m[i].z;
			current_cells[icell].ele -= ct[icell].v_m[i].Dzc * dV_dcell * F_Re3 /tk_x2 *
				(1 - ct[icell].dl_s) * ct[icell].v_m[i].z;
			if (ct[icell].Dz2c_dl > 0)
			{
				dum2 = -c_dl * ct[icell].v_m[i].Dzc_dl * dV_dcell * F_Re3 / tk_x2;
				if ((ct[icell].J_ij[i].tot1 <= 0 && dum <= 0)
					|| (ct[icell].J_ij[i].tot1 > 0 && dum > 0))
				{
					//current_cells[icell].dif += ct[icell].v_m[i].o_c * dum * 2 / 
					//	(ct[icell].visc1 + ct[icell].visc2) * ct[icell].dl_s * ct[icell].v_m[i].z;
					current_cells[icell].ele += ct[icell].v_m[i].o_c * dum2 * 2 / 
						(ct[icell].visc1 + ct[icell].visc2) * ct[icell].dl_s * ct[icell].v_m[i].z;
				}
			}
		}
		else if (dV_dcell)
			ct[icell].J_ij[i].tot1 -= ct[icell].v_m[i].Dzc * dV * F_Re3 / tk_x2 *
			(1 - ct[icell].dl_s);
		...
		if (stagnant)
			...
			ct[icell].J_ij[i].tot1 *= ct[icell].A_ij * DDt;
	...
	if (find_current)
	{
		...
		// ** create label to use V_M when f_c = 1
		//V_M = (struct V_M *) free_check_null(V_M);
		return(il_calcs);
	}
	else if (dV_dcell)
		current_A = current_x * F_C_MOL / DDt;
...
/* (modified) Jones-Dole eqn for viscosity:
	  ...
	  B_i = b0 + b1*exp(b2 * tc), b0..2 in Jones_Dole[0..2], read in SOLUTION_SPECIES (b0_here = b0_25C_read - b1*exp(b2*25))
	  ...
	  f_z = (z * z + |z|) / 2, the contribution of the ion to mu_x
 ...
 */
	LDBLE D1, D2, z1, z2, m_plus, m_min, eq_plus, eq_min, eq_dw_plus, eq_dw_min, t1, t2, t3, ta;
	LDBLE A, psi, Bc = 0, Dc = 0, Bnc = 0, Dw, l_z, f_z, lm, V_an, m_an, V_Cl, tc, s_v, pt;

	m_plus = m_min = eq_plus = eq_min = eq_dw_plus = eq_dw_min = V_an = m_an = V_Cl = ta = s_v = 0;
	...
	for (i = 0; i < count_s_x; i++)
	{
		...
		if (s_x[i]->Jones_Dole[0] || s_x[i]->Jones_Dole[1] || s_x[i]->Jones_Dole[3])
		{
			...
			if (l_z)
			{
				...
				f_z = (l_z * l_z + l_z) / 2;
				// pressure
				if (s_x[i]->Jones_Dole[7])
					pt = (1 + s_x[i]->Jones_Dole[7] * exp(s_x[i]->Jones_Dole[8] * tc) * (patm_x - 1));
				else
					pt = 1.0;
				...
				Dc += (s_x[i]->Jones_Dole[3] * exp(s_x[i]->Jones_Dole[4] * tc)) *
					 t1 * (pow(mu_x, s_x[i]->Jones_Dole[5])*(1 + t2) + 
					 pow(t1 * f_z, s_x[i]->Jones_Dole[5])) / (2 + t2) *
					 pt;
			...
			{
				// Neutral species, Bnc = (B * c + D * c^dn ) * Vm ...
				if (!s_v)
					s_v = calc_solution_volume();
				t1 = s_x[i]->moles / s_v;
				t3 = s_x[i]->Jones_Dole[0] + 
					s_x[i]->Jones_Dole[1] * exp(s_x[i]->Jones_Dole[2] * tc);
				t3 *= t1;
				if (s_x[i]->Jones_Dole[3] && s_x[i]->Jones_Dole[5])
				{
					// from 0 - 2.8 M sucrose in 0 - 5.6 M NaCl...
					if (s_x[i]->Jones_Dole[5] >= 2)
						t2 = 0.07;
					else
						t2 = 0;
					t3 += (s_x[i]->Jones_Dole[3] * exp(s_x[i]->Jones_Dole[4] * tc)) *
						t1 * (pow(mu_x, (s_x[i]->Jones_Dole[5] - 1)) * (0 + t2) +
						pow(t1, s_x[i]->Jones_Dole[5])) / (1 + t2);
				}
				// pressure
				if (s_x[i]->Jones_Dole[7])
					pt = (1 + s_x[i]->Jones_Dole[7] * exp(s_x[i]->Jones_Dole[8] * tc) * (patm_x - 1));
					...
					pt = 1.0;
				Bnc += t3 * s_x[i]->logk[vm_tc] / 1e3 * pt;
			}
		}
		//s_v = 0;
		// parms for A...
		...
		if ((l_z = s_x[i]->z) == 0)
			continue;
		Dw *= (viscos_0_25 / viscos_0 * tk_x / 298.15);
		if (s_x[i]->dw_t)
		...
		else
		{
			...
			{
				V_an += s_x[i]->logk[vm_tc] * s_x[i]->moles;
				// pressure
				if (s_x[i]->Jones_Dole[7])
					pt = (1 + s_x[i]->Jones_Dole[7] * exp(s_x[i]->Jones_Dole[8] * tc) * (patm_x - 1));
				else
					pt = 1.0;
				ta += s_x[i]->Jones_Dole[6] * s_x[i]->moles * pt;
				m_an += s_x[i]->moles;
			}
			// anions for A...
			m_min += s_x[i]->moles;
	...
	if (!V_Cl)
		V_Cl = calc_vm_Cl();
	if (V_an && ta)
		viscos += viscos_0 * ((2 - ta * V_an / V_Cl) * (Bc + Dc) + Bnc);
	else
		viscos += viscos_0 * (Bc + Dc + Bnc);
