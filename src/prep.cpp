 ...
 */
	...
	x[count_unknowns]->description = "gas moles";
 ...
 */
	...
	for (size_t j = 0; j < ss_ptrs.size(); j++)
	{
		for (size_t i = 0; i < ss_ptrs[j]->Get_ss_comps().size(); i++)
		{
			...
			x[count_unknowns]->ln_moles = log(x[count_unknowns]->moles);
			x[count_unknowns]->ss_name = ss_ptrs[j]->Get_name();
			x[count_unknowns]->ss_ptr =  ss_ptrs[j];
			x[count_unknowns]->ss_comp_name = comp_ptr->Get_name();
...
/* ---------------------------------------------------------------------- */
{
	...
	for (size_t i = 0; i < use.Get_surface_ptr()->Get_surface_comps().size(); i++)
	{
		...
		for (jit = comp_ptr->Get_totals().begin(); jit != comp_ptr->Get_totals().end(); jit++)
		{
			...
			x[count_unknowns]->description = jit->first.c_str();
			x[count_unknowns]->number = count_unknowns;
			x[count_unknowns]->surface_comp = comp_ptr->Get_formula();
			x[count_unknowns]->master = master_ptr_list;
			x[count_unknowns]->master[0]->unknown = x[count_unknowns];
 ...
 */
		...
		x[count_unknowns]->description = it->first.c_str();
 ...
 */
	...
	//if (pitzer_model == FALSE && sit_model == FALSE)
	{
		...
		x[count_unknowns]->description = "Mu";
	...
	ah2o_unknown->description = "A(H2O)";
 ...
 */
		...
		mass_hydrogen_unknown->description = "Hydrogen";
 ...
 */
		...
		mass_oxygen_unknown->description = "Oxygen";
	...
	for (int i = 0; i < (int)this->s_x.size(); i++)
	{
		...
		if (s_x[i]->logk[Logk::vma1])
		{
			...
			s_x[i]->rxn_x.logk_x[Logk::vm_tc] = 
				...
				(s_x[i]->rxn.logk_original[Logk::vma3] + s_x[i]->rxn.logk_original[Logk::vma4] / pb_s) / TK_s -
				s_x[i]->rxn.logk_original[Logk::wref] * QBrn;
			...
			{
				/* the ionic strength term * I^0.5... */
				if (s_x[i]->rxn.logk_original[Logk::b_Av] < 1e-5)
					s_x[i]->rxn_x.logk_x[Logk::vm_tc] += 
				...
				else
				{
					...
					s_x[i]->rxn_x.logk_x[Logk::vm_tc] += 
						...
						sqrt_mu / (1 + s_x[i]->rxn.logk_original[Logk::b_Av] * DH_B * sqrt_mu);
				}
				/* plus the volume terms * I... */
				if (s_x[i]->rxn.logk_original[Logk::vmi1] != 0.0 || 
					s_x[i]->rxn.logk_original[Logk::vmi2] != 0.0 || 
					s_x[i]->rxn.logk_original[Logk::vmi3] != 0.0)
				{
					double bi = 
						s_x[i]->rxn.logk_original[Logk::vmi1] + 
						s_x[i]->rxn.logk_original[Logk::vmi2] / TK_s + 
						s_x[i]->rxn.logk_original[Logk::vmi3] * TK_s;
					if (s_x[i]->rxn.logk_original[Logk::vmi4] == 1.0)
						s_x[i]->rxn_x.logk_x[Logk::vm_tc] += bi * mu_x;
					else
						s_x[i]->rxn_x.logk_x[Logk::vm_tc] += 
							bi * pow(mu_x, s_x[i]->rxn.logk_original[Logk::vmi4]);
				}
			}
		}
		else if (s_x[i]->millero[0])
		{
			...
			{
				/* the ionic strength terms... */
				s_x[i]->rxn_x.logk_x[Logk::vm_tc] += s_x[i]->z * s_x[i]->z * 0.5 * DH_Av * sqrt_mu +
					(s_x[i]->millero[3] + tc * (s_x[i]->millero[4] + tc * s_x[i]->millero[5])) * mu_x;
			}
		}
		else
...
/* ---------------------------------------------------------------------- */
{
	...
	LDBLE lk = l_logk[Logk::logK_T0] 
		- l_logk[Logk::delta_h] * (298.15 - tempk) / (LOG_10 * me * 298.15)
		+ l_logk[Logk::T_A1]
		+ l_logk[Logk::T_A2] * tempk
		+ l_logk[Logk::T_A3] / tempk
		+ l_logk[Logk::T_A4] * log10(tempk)
		+ l_logk[Logk::T_A5] / (tempk * tempk)
		+ l_logk[Logk::T_A6] * tempk * tempk;
	if (delta_p > 0)
		/* cm3 * J /mol = 1e-9 m3 * kJ /mol */
		lk -= l_logk[Logk::delta_v] * 1E-9 * delta_p / (LOG_10 * me);
	return lk;
}
/* ---------------------------------------------------------------------- */
double Phreeqc::
k_calc(std::vector<double>& l_logk, double tempk, double presPa)
/* ---------------------------------------------------------------------- */
{
	/*
	 *   Calculates log k at specified temperature and pressure
 ...
 */
	if (use.Get_ss_assemblage_ptr() != NULL)
	{
		...
		for (size_t j = 0; j < ss_ptrs.size(); j++)
		{
			last_model.ss_assemblage[j] = ss_ptrs[j]->Get_name();
		}
	}
	else
	{
 ...
 */
	if (use.Get_surface_ptr() != NULL)
	{
		...
		{
			last_model.surface_comp[i] = use.Get_surface_ptr()->Get_surface_comps()[i].Get_formula();
		}
		/* charge */
		...
		{
			last_model.surface_charge[i] = use.Get_surface_ptr()->Get_surface_charges()[i].Get_name();
		}
 ...
 */
	if (use.Get_ss_assemblage_ptr() != NULL)
	{
		...
		for (size_t i = 0; i < ss_ptrs.size(); i++)
		{
			if (last_model.ss_assemblage[i] != ss_ptrs[i]->Get_name())
			{
				return (FALSE);
			}
		}
 ...
 */
	if (use.Get_pp_assemblage_ptr() != NULL)
	{
		...
		for ( ; it != pp_assemblage_ptr->Get_pp_assemblage_comps().end(); it++)
		{
			...
			if (last_model.add_formula[i] != it->second.Get_add_formula())
			{
				return (FALSE);
			}
			i++;
	...
	if (use.Get_surface_ptr() != NULL)
	{
		...
		for (i = 0; i < (int) use.Get_surface_ptr()->Get_surface_comps().size(); i++)
		{
			if (last_model.surface_comp[i] !=
				use.Get_surface_ptr()->Get_surface_comps()[i].Get_formula())
				return (FALSE);
			if (use.Get_surface_ptr()->Get_surface_comps()[i].Get_phase_name().size() > 0)
			{
				cxxPPassemblage *pp_ptr = Utilities::Rxn_find(Rxn_pp_assemblage_map, use.Get_n_surface_user());
		...
		for (i = 0; i < (int) use.Get_surface_ptr()->Get_surface_charges().size(); i++)
		{
			if (last_model.surface_charge[i] !=
				use.Get_surface_ptr()->Get_surface_charges()[i].Get_name())
				return (FALSE);
		}
	}
	else
