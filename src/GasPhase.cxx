...
void
cxxGasPhase::mpi_pack(Dictionary & dictionary, std::vector < int >&ints, std::vector < double >&doubles)
{
	...
	for (size_t i = 0; i < this->gas_comps.size(); i++)
	{
		this->gas_comps[i].mpi_pack(dictionary, ints, doubles);
	}
...
void
cxxGasPhase::mpi_unpack(Dictionary & dictionary, std::vector < int >&ints, 
	std::vector < double >&doubles, int &ii, int &dd)
{
	...
	{
		cxxGasComp gc;
		gc.mpi_unpack(dictionary, ints, doubles, ii, dd);
