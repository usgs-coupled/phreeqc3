...
/* ---------------------------------------------------------------------- */
{
	...
	*/
	...
	for (i = 0; i < count_cations - 1; i++)
	{
		...
		{
			sprintf(line, "%s %s 1", spec[i]->name.c_str(), spec[j]->name.c_str());
	...
	for (i = 0; i < (int)pitz_params.size(); i++)
	{
		for (j = 0; j < 3; j++)
		{
			if (pitz_params[i]->species[j].size() == 0)
				continue;
			pitz_params[i]->ispec[j] = ISPEC(pitz_params[i]->species[j].c_str());
	...
	/* remake map */
	{
		...
		{	
			...
			for (int i = 0; i < 3; i++)
			{
				if (pitz_params[j]->species[i].size() > 0) header.insert(pitz_params[j]->species[i]);
			}
		...
		for (i = 0; i < (int)this->s_x.size(); i++)
		{
			...
			case 4:			/* Exchange */
				...
				for (j = 1; !s_x[i]->rxn_x.Get_tokens()[j].Get_end(); j++)
				{
					...
					{
						s_x[i]->alk = s_x[i]->rxn_x.Get_tokens()[j].Get_s()->primary->unknown->moles;
						break;
					}
				}
				/*
				...
				if (use.Get_exchange_ptr()->Get_pitzer_exchange_gammas())
				{
					...
					for (j = 1; !s_x[i]->rxn_x.Get_tokens()[j].Get_end(); j++)
					{
						...
						s_x[i]->lg += coef * s_x[i]->rxn_x.Get_tokens()[j].Get_s()->lg;
					}
				}
