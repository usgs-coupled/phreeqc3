	...
	* ---------------------------------------------------------------------- */
	...
	for (int i = 0; i < (int)pSrc->s.size(); i++)
	{
		...
		*s_ptr = *pSrc->s[i];
		...
		s_ptr->rxn.Get_add_logk().resize(pSrc->s[i]->rxn.Get_add_logk().size());
		for (size_t j = 0; j < s_ptr->rxn.Get_add_logk().size(); j++)
		{
			s_ptr->rxn.Get_add_logk()[j].coef = pSrc->s[i]->rxn.Get_add_logk()[j].coef;
			s_ptr->rxn.Get_add_logk()[j].name = pSrc->s[i]->rxn.Get_add_logk()[j].name;
		}
		//next_elt
	...
	*---------------------------------------------------------------------- */
	for (int i = 0; i < (int)pSrc->phases.size(); i++)
	{
		...
		*phase_ptr = *pSrc->phases[i];
		...
		//add_logk
		phase_ptr->rxn.Get_add_logk().resize(pSrc->phases[i]->rxn.Get_add_logk().size());
		for (size_t j = 0; j < phase_ptr->rxn.Get_add_logk().size(); j++)
		{
			phase_ptr->rxn.Get_add_logk()[j].coef = pSrc->phases[i]->rxn.Get_add_logk()[j].coef;
			phase_ptr->rxn.Get_add_logk()[j].name = pSrc->phases[i]->rxn.Get_add_logk()[j].name;
		}
		//next_elt
