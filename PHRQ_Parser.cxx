// Parser.cpp: implementation of the PHRQ_Parser class.
//
//////////////////////////////////////////////////////////////////////
#ifdef _DEBUG
...
PHRQ_Parser::PHRQ_Parser(PHREEQC_PTR_ARG_COMMA std::istream & input):m_input_stream(input), m_output_stream(std::cout), m_error_stream(std::cerr),
m_input_error(0),
...
PHRQ_Parser::PHRQ_Parser(PHREEQC_PTR_ARG_COMMA std::istream & input, std::ostream & output):m_input_stream(input), m_output_stream(output), m_error_stream(std::cerr),
m_input_error(0),
m_next_keyword(KT_NONE)
{
	m_line_save.reserve(80);
...
PHRQ_Parser::PHRQ_Parser(PHREEQC_PTR_ARG_COMMA std::istream & input, std::ostream & output, std::ostream & error):m_input_stream(input), m_output_stream(output), m_error_stream(error),
m_input_error(0),
m_next_keyword(KT_NONE)
{
	m_line_save.reserve(80);
...
PHRQ_Parser::~PHRQ_Parser()
{
}

PHRQ_Parser::LINE_TYPE PHRQ_Parser::check_line(const std::string & str,
...
PHRQ_Parser::LINE_TYPE PHRQ_Parser::get_line()
{
	PHRQ_Parser::LINE_TYPE return_value = LT_EMPTY;
...
/**
        ...
        returns:
                ...
                OK otherwise
*/
PHRQ_Parser::LINE_TYPE PHRQ_Parser::get_logical_line()
{
	int
		j;
	unsigned int
...
//bool PHRQ_Parser::check_key(const std::string::iterator ptr)
bool
PHRQ_Parser::check_key(std::string::iterator begin, std::string::iterator end)
{
...
PHRQ_Parser::STATUS_TYPE PHRQ_Parser::check_units(std::string & tot_units,
										  bool alkalinity,
...
PHRQ_Parser::TOKEN_TYPE PHRQ_Parser::token_type(const std::string & token)
{
	if (!token.empty())
	{
		if (::isupper(token[0]))
		{
			return PHRQ_Parser::TT_UPPER;
		}
		else if (::islower(token[0]))
		{
			return PHRQ_Parser::TT_LOWER;
		}
		else if (::isdigit(token[0]) || token[0] == '.' || token[0] == '-')
		{
			return PHRQ_Parser::TT_DIGIT;
		}
		else
		{
			assert(!::isspace(token[0]));
			return PHRQ_Parser::TT_UNKNOWN;
		}
	}
	else
	{
		return PHRQ_Parser::TT_EMPTY;
	}
}

PHRQ_Parser::TOKEN_TYPE PHRQ_Parser::peek_token()
{
...
PHRQ_Parser::TOKEN_TYPE PHRQ_Parser::copy_token(std::string & token,
...
PHRQ_Parser::FIND_TYPE PHRQ_Parser::find_option(const std::string & item, int *n,
...
// OPTION_TYPE PHRQ_Parser::get_option(const std::vector<std::string>& opt_list, std::string::iterator& next_char)
int
PHRQ_Parser::get_option(const std::vector < std::string > &opt_list,
					std::string::iterator & next_char)
{
	...
	else if (lt == LT_OPTION)
	{
		...
		if (find_option(option, &opt, opt_list, false) == PHRQ_Parser::FT_OK)
		{
			j = opt;
...
int
PHRQ_Parser::error_msg(const char *err_str, ONERROR_TYPE ot)
{
...
int
PHRQ_Parser::warning_msg(const char *err_str)
{
...
PHRQ_Parser::STATUS_TYPE PHRQ_Parser::get_elt(std::string::iterator & begin,
...
PHRQ_Parser::STATUS_TYPE PHRQ_Parser::parse_couple(std::string & token)
{
...
//PHRQ_Parser::STATUS_TYPE PHRQ_Parser::addPair(std::map < char *, double,
...
PHRQ_Parser::STATUS_TYPE PHRQ_Parser::addPair(std::map < std::string, double >&totals,
									  std::istream::pos_type & pos)
{
	std::string token;
	//char * ctoken;
...
int
PHRQ_Parser::getOptionFromLastLine(const std::vector < std::string > &opt_list,
							   std::string::iterator & next_char)
{
	//
	// Read a line and check for options
...
PHRQ_Parser::TOKEN_TYPE PHRQ_Parser::copy_title(std::string & token,
										std::string::iterator & begin,
										std::string::iterator & end)
{
	if (begin != end)
...
bool PHRQ_Parser::get_true_false(std::istream::pos_type & pos, bool def)
{
	std::string token;
...
PHRQ_Parser::TOKEN_TYPE PHRQ_Parser::get_rest_of_line(std::string &token)
{
	token.clear();
	std::istringstream::pos_type pos = m_line_iss.tellg();
	int j;
...
PHRQ_Parser::TOKEN_TYPE PHRQ_Parser::parse_delimited(std::string & source, std::string & result,
										const std::string& t = " \t")
{
